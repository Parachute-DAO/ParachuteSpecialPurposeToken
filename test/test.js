...
describe('Parachute SPT Calls', async () => {
    ...
    it('creates two new single asks', async () => {
        ...
        .withArgs('0', assetAmount, strike, price, expiry);
        // test the following:
        ...
        const call = await sptCall.calls(0);
     ...
     it('creates bulk newAsks', async () => {
        let priorBalance = await par.balanceOf(sptCall.address);
        let expiry = constants.IN_FIVE_MINS;
    ...
    it('buys a new option', async () => {
        // buy call option 0
        let call = await sptCall.calls(0);
        const buyReceipt = await sptCall.connect(otherA).buyNewOption('0');
        await expect(buyReceipt).to.emit(sptCall, 'NewOptionBought').withArgs('0')
        .to.emit(spt, 'Transfer').withArgs(otherA.address, constants.ZERO_ADDRESS, call[4]); 
        call = await sptCall.calls(0);
        expect(call[6]).to.eq(true);
        expect(call[7]).to.eq(false);
        expect(call[8]).to.eq(otherA.address);
        expect(call[9]).to.eq(false);
        //test that its moved from new array to open array
        expect(await sptCall.openCalls('0')).to.eq('0');
    ...
    it('physically exercises an option', async () => {
        const exercise = await sptCall.connect(otherA).exercise('0');
        await expect(exercise).to.emit(sptCall, 'OptionExercised').withArgs('0', false);
        expect(await sptCall.balanceOf(otherA.address)).to.eq(0);
    });
    it('cash closes an option', async() => {
        await sptCall.connect(otherB).buyNewOption('1');
        let call = await sptCall.calls(1);
        const cashClose = sptCall.connect(otherB).cashClose('1', true);
        await expect(cashClose).to.emit(sptCall, 'OptionExercised').withArgs('1', true);
    });
